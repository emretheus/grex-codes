...
import { forgejoGetIssue, type IssueDetail } from "@/lib/api";
import type { ComposerInsertTarget } from "@/lib/composer-insert";
import { useComposerInsert } from "@/lib/composer-insert-context";
import { grexQueryKeys } from "@/lib/query-client";
import type { ContextCard, ForgejoIssueMeta } from "@/lib/sources/types";
import {
	DetailErrorState,
	DetailLoadingState,
	formatRelativeTime,
...
export function ForgejoIssueView({
	...
	// `connectionId` selects which instance's credentials fetch it.
	const detailQuery = useQuery({
		queryKey: grexQueryKeys.forgejoIssueDetail(connectionId, card.id),
		queryFn: () => forgejoGetIssue({ connectionId, issueId: card.id }),
	...
	const handleStartWorkspace = () => {
		...
		insertIntoComposer(
			buildForgejoStartInsert(card, detail, appendContextTarget),
		);
...
/** Build the composer insert that seeds a new workspace from a Forgejo
 *  issue: a custom tag whose submit text carries the issue header
 *  (title, URL, repo, labels) plus the markdown body, so the agent's
 *  first turn has the whole task in context. */
export function buildForgejoStartInsert(
	card: ContextCard,
	detail: IssueDetail | null,
	target?: ComposerInsertTarget,
) {
	const meta =
		card.meta.type === "forgejo" ? (card.meta as ForgejoIssueMeta) : null;
	const headerLines = [
		`Forgejo issue: ${card.title}`,
